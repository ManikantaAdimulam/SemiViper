...
class UserManagementRouter {
    ...
    ///
    func assembleExampleScreen() -> UITabBarController {
        guard let vc = ExampleModuleRouter(with: networkService).assembelTabBarController() else {
            DDLogError("Unable to get SignUpViewController.")
            fatalError("Unable to get SignUpViewController.")}
        return vc
    }
    ...
    func navigateToExampleModule() {
        let exampleModuleVC = assembleExampleScreen()
        navigationController?.pushViewController(exampleModuleVC, animated: true)
    }
}
